...
// toJSON converts a value to a JSON string for embedding in test code
...
// hasTerraform checks if a SyntaxTest has a non-nil terraform field
func hasTerraform(v interface{}) bool {
	return v != nil
}
...
// TestSpec{{toCamelCase .Command.Name}}RTXSyntax validates that RTX commands in spec are well-formed
func TestSpec{{toCamelCase .Command.Name}}RTXSyntax(t *testing.T) {
...
// TestSpec{{toCamelCase .Command.Name}}RTXTerraformMapping validates RTX command to Terraform value mappings
func TestSpec{{toCamelCase .Command.Name}}RTXTerraformMapping(t *testing.T) {
...
{{- range .Command.SyntaxTests}}
		{
			name:          "{{.Name}}",
			rtxCommand:    ` + "`" + `{{.RTX}}` + "`" + `,
			terraformJSON: ` + "`" + `{{toJSON .Terraform}}` + "`" + `,
			parseOnly:     {{.ParseOnly}},
			buildOnly:     {{.BuildOnly}},
		},
{{- end}}
