// Code generated by specgen. DO NOT EDIT.
// Source: specs/bgp/config.yaml
// Command: bgp_config
// Reference: RTX Command Reference

package parsers

import (
	"encoding/json"
	"strings"
	"testing"
)

// TestSpecBgpConfigRTXSyntax validates that RTX commands in spec are well-formed
func TestSpecBgpConfigRTXSyntax(t *testing.T) {
	// This test validates that all RTX command strings in the spec are well-formed
	// and follow expected patterns

	testCases := []struct {
		name       string
		rtxCommand string
		parseOnly  bool
		buildOnly  bool
	}{
		{
			name:       "bgp_use_on",
			rtxCommand: `bgp use on`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_use_off",
			rtxCommand: `bgp use off`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_asn_2byte",
			rtxCommand: `bgp autonomous-system 65001`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_asn_min",
			rtxCommand: `bgp autonomous-system 100`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_router_id",
			rtxCommand: `bgp router id 192.168.1.1`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_router_id_loopback",
			rtxCommand: `bgp router id 10.0.0.1`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp neighbor <n> <as> <ip>
			name:       "bgp_neighbor",
			rtxCommand: `bgp neighbor 1 65002 203.0.113.1`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_neighbor_second",
			rtxCommand: `bgp neighbor 2 65003 203.0.113.2`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp neighbor <n> <as> <ip> hold-time=<sec>
			name:       "bgp_neighbor_holdtime",
			rtxCommand: `bgp neighbor 1 65002 192.168.1.2 hold-time=90`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp neighbor <n> <as> <ip> local-address=<ip>
			name:       "bgp_neighbor_local_address",
			rtxCommand: `bgp neighbor 1 65002 192.168.1.2 local-address=192.168.1.1`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp neighbor <n> <as> <ip> passive=on
			name:       "bgp_neighbor_passive",
			rtxCommand: `bgp neighbor 1 65002 192.168.1.2 passive=on`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp neighbor pre-shared-key <n> text <password>
			name:       "bgp_neighbor_preshared_key",
			rtxCommand: `bgp neighbor pre-shared-key 1 text mysecret`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			// Reference: bgp import filter <n> include <prefix>/<cidr>
			name:       "bgp_network",
			rtxCommand: `bgp import filter 1 include 192.168.0.0/16`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_network_24",
			rtxCommand: `bgp import filter 1 include 10.0.0.0/24`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_import_static",
			rtxCommand: `bgp import from static`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "bgp_import_connected",
			rtxCommand: `bgp import from connected`,
			parseOnly:  false,
			buildOnly:  false,
		},
		{
			name:       "delete_bgp_neighbor",
			rtxCommand: `no bgp neighbor 1`,
			parseOnly:  false,
			buildOnly:  true,
		},
		{
			name:       "delete_bgp_network",
			rtxCommand: `no bgp import filter 1`,
			parseOnly:  false,
			buildOnly:  true,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Validate RTX command is not empty
			if strings.TrimSpace(tc.rtxCommand) == "" {
				t.Errorf("RTX command should not be empty")
			}

			// Validate command doesn't have trailing/leading whitespace issues
			trimmed := strings.TrimSpace(tc.rtxCommand)
			if tc.rtxCommand != trimmed && !strings.Contains(tc.rtxCommand, "\n") {
				t.Errorf("RTX command has unexpected whitespace: %q", tc.rtxCommand)
			}

			// Log the command for visibility
			if !tc.buildOnly {
				t.Logf("Parse test: %s", tc.rtxCommand)
			}
			if !tc.parseOnly {
				t.Logf("Build test: %s", tc.rtxCommand)
			}
		})
	}
}

// TestSpecBgpConfigSyntaxCoverage documents the syntax patterns covered by this spec
func TestSpecBgpConfigSyntaxCoverage(t *testing.T) {
	// This test documents all syntax patterns defined in the spec
	// Each pattern should have corresponding implementation in the parser

	syntaxPatterns := []struct {
		name        string
		rtxCommand  string
		parseOnly   bool
		buildOnly   bool
		description string
	}{
		{
			name:        "bgp_use_on",
			rtxCommand:  `bgp use on`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_use_off",
			rtxCommand:  `bgp use off`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_asn_2byte",
			rtxCommand:  `bgp autonomous-system 65001`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_asn_min",
			rtxCommand:  `bgp autonomous-system 100`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_router_id",
			rtxCommand:  `bgp router id 192.168.1.1`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_router_id_loopback",
			rtxCommand:  `bgp router id 10.0.0.1`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_neighbor",
			rtxCommand:  `bgp neighbor 1 65002 203.0.113.1`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp neighbor <n> <as> <ip>",
		},
		{
			name:        "bgp_neighbor_second",
			rtxCommand:  `bgp neighbor 2 65003 203.0.113.2`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_neighbor_holdtime",
			rtxCommand:  `bgp neighbor 1 65002 192.168.1.2 hold-time=90`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp neighbor <n> <as> <ip> hold-time=<sec>",
		},
		{
			name:        "bgp_neighbor_local_address",
			rtxCommand:  `bgp neighbor 1 65002 192.168.1.2 local-address=192.168.1.1`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp neighbor <n> <as> <ip> local-address=<ip>",
		},
		{
			name:        "bgp_neighbor_passive",
			rtxCommand:  `bgp neighbor 1 65002 192.168.1.2 passive=on`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp neighbor <n> <as> <ip> passive=on|off",
		},
		{
			name:        "bgp_neighbor_preshared_key",
			rtxCommand:  `bgp neighbor pre-shared-key 1 text mysecret`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp neighbor pre-shared-key <n> text <password>",
		},
		{
			name:        "bgp_network",
			rtxCommand:  `bgp import filter 1 include 192.168.0.0/16`,
			parseOnly:   false,
			buildOnly:   false,
			description: "Reference: bgp import filter <n> include <prefix>/<cidr>",
		},
		{
			name:        "bgp_network_24",
			rtxCommand:  `bgp import filter 1 include 10.0.0.0/24`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_import_static",
			rtxCommand:  `bgp import from static`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "bgp_import_connected",
			rtxCommand:  `bgp import from connected`,
			parseOnly:   false,
			buildOnly:   false,
			description: "",
		},
		{
			name:        "delete_bgp_neighbor",
			rtxCommand:  `no bgp neighbor 1`,
			parseOnly:   false,
			buildOnly:   true,
			description: "",
		},
		{
			name:        "delete_bgp_network",
			rtxCommand:  `no bgp import filter 1`,
			parseOnly:   false,
			buildOnly:   true,
			description: "",
		},
	}

	t.Logf("Total syntax patterns: %d", len(syntaxPatterns))

	parseOnlyCount := 0
	buildOnlyCount := 0
	bidirectionalCount := 0

	for _, p := range syntaxPatterns {
		if p.parseOnly {
			parseOnlyCount++
		} else if p.buildOnly {
			buildOnlyCount++
		} else {
			bidirectionalCount++
		}
	}

	t.Logf("Parse-only patterns: %d", parseOnlyCount)
	t.Logf("Build-only patterns: %d", buildOnlyCount)
	t.Logf("Bidirectional patterns: %d", bidirectionalCount)
}

// TestSpecBgpConfigBoundaryCoverage documents the boundary values covered by this spec
func TestSpecBgpConfigBoundaryCoverage(t *testing.T) {
	// This test documents all boundary value tests defined in the spec
	boundaryParams := []string{
		"asn",
		"hold_time",
		"neighbor_id",
	}

	t.Logf("Parameters with boundary tests: %d", len(boundaryParams))
	for _, param := range boundaryParams {
		t.Logf("  - %s", param)
	}
}

// TestSpecBgpConfigRTXTerraformMapping validates RTX command to Terraform value mappings
func TestSpecBgpConfigRTXTerraformMapping(t *testing.T) {
	// This test validates that each RTX command has a corresponding expected Terraform value
	// and vice versa. It ensures the spec file correctly documents the bidirectional mapping.

	testCases := []struct {
		name          string
		rtxCommand    string
		terraformJSON string
		parseOnly     bool
		buildOnly     bool
	}{
		{
			name:          "bgp_use_on",
			rtxCommand:    `bgp use on`,
			terraformJSON: `{"enabled":true}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_use_off",
			rtxCommand:    `bgp use off`,
			terraformJSON: `{"enabled":false}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_asn_2byte",
			rtxCommand:    `bgp autonomous-system 65001`,
			terraformJSON: `{"asn":"65001"}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_router_id",
			rtxCommand:    `bgp router id 192.168.1.1`,
			terraformJSON: `{"router_id":"192.168.1.1"}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_router_id_loopback",
			rtxCommand:    `bgp router id 10.0.0.1`,
			terraformJSON: `{"router_id":"10.0.0.1"}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor",
			rtxCommand:    `bgp neighbor 1 65002 203.0.113.1`,
			terraformJSON: `{"neighbors":[{"id":1,"ip":"203.0.113.1","remote_as":"65002"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor_second",
			rtxCommand:    `bgp neighbor 2 65003 203.0.113.2`,
			terraformJSON: `{"neighbors":[{"id":2,"ip":"203.0.113.2","remote_as":"65003"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor_holdtime",
			rtxCommand:    `bgp neighbor 1 65002 192.168.1.2 hold-time=90`,
			terraformJSON: `{"neighbors":[{"hold_time":90,"id":1,"ip":"192.168.1.2","remote_as":"65002"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor_local_address",
			rtxCommand:    `bgp neighbor 1 65002 192.168.1.2 local-address=192.168.1.1`,
			terraformJSON: `{"neighbors":[{"id":1,"ip":"192.168.1.2","local_address":"192.168.1.1","remote_as":"65002"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor_passive",
			rtxCommand:    `bgp neighbor 1 65002 192.168.1.2 passive=on`,
			terraformJSON: `{"neighbors":[{"id":1,"ip":"192.168.1.2","passive":true,"remote_as":"65002"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_neighbor_preshared_key",
			rtxCommand:    `bgp neighbor pre-shared-key 1 text mysecret`,
			terraformJSON: `{"neighbors":[{"id":1,"password":"mysecret"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_network",
			rtxCommand:    `bgp import filter 1 include 192.168.0.0/16`,
			terraformJSON: `{"networks":[{"mask":"255.255.0.0","prefix":"192.168.0.0"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_network_24",
			rtxCommand:    `bgp import filter 1 include 10.0.0.0/24`,
			terraformJSON: `{"networks":[{"mask":"255.255.255.0","prefix":"10.0.0.0"}]}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_import_static",
			rtxCommand:    `bgp import from static`,
			terraformJSON: `{"redistribute_static":true}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "bgp_import_connected",
			rtxCommand:    `bgp import from connected`,
			terraformJSON: `{"redistribute_connected":true}`,
			parseOnly:     false,
			buildOnly:     false,
		},
		{
			name:          "delete_bgp_neighbor",
			rtxCommand:    `no bgp neighbor 1`,
			terraformJSON: `{"neighbor_id":1}`,
			parseOnly:     false,
			buildOnly:     true,
		},
		{
			name:          "delete_bgp_network",
			rtxCommand:    `no bgp import filter 1`,
			terraformJSON: `{"filter_id":1}`,
			parseOnly:     false,
			buildOnly:     true,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Validate RTX command is not empty
			if strings.TrimSpace(tc.rtxCommand) == "" {
				t.Errorf("RTX command should not be empty")
				return
			}

			// build_only tests (like delete commands) may have empty terraform values
			// because they represent commands that don't have a direct terraform mapping
			if tc.buildOnly {
				t.Logf("Direction: Terraform -> RTX only (build)")
				t.Logf("RTX: %s", tc.rtxCommand)
				t.Logf("Terraform: %s (build_only, may be empty)", tc.terraformJSON)
				return
			}

			// Validate terraform JSON is present and valid for non-build_only tests
			if tc.terraformJSON == "null" || tc.terraformJSON == "" {
				t.Errorf("Terraform value is missing for RTX command: %s", tc.rtxCommand)
				return
			}

			var tfValue interface{}
			if err := json.Unmarshal([]byte(tc.terraformJSON), &tfValue); err != nil {
				t.Errorf("Invalid terraform JSON: %v\nJSON: %s", err, tc.terraformJSON)
				return
			}

			// Log the mapping for visibility
			t.Logf("RTX: %s", tc.rtxCommand)
			t.Logf("Terraform: %s", tc.terraformJSON)

			// Validate that terraform value contains expected fields (only for non-build_only tests)
			if tfMap, ok := tfValue.(map[string]interface{}); ok {
				if len(tfMap) == 0 {
					t.Errorf("Terraform value is empty map for RTX command: %s", tc.rtxCommand)
					return
				}
			}

			// Check mapping direction
			if tc.parseOnly {
				t.Logf("Direction: RTX -> Terraform only (parse)")
			} else {
				t.Logf("Direction: Bidirectional (parse and build)")
			}
		})
	}
}

// TestSpecBgpConfigModelCompatibility tests router model compatibility
func TestSpecBgpConfigModelCompatibility(t *testing.T) {
	// All known RTX router models
	allModels := []string{
		"vRX",
		"RTX5000",
		"RTX3510",
		"RTX3500",
		"RTX1300",
		"RTX1220",
		"RTX1210",
		"RTX840",
		"RTX830",
		"RTX810",
		"NVR700W",
		"NVR510",
		"NVR500",
	}

	// Models that support this command according to the spec
	supportedModels := []string{
		"vRX",
		"RTX5000",
		"RTX3510",
		"RTX3500",
		"RTX1300",
		"RTX1220",
		"RTX1210",
		"RTX840",
		"RTX830",
	}

	// Build a map for quick lookup
	supportedMap := make(map[string]bool)
	for _, m := range supportedModels {
		supportedMap[m] = true
	}

	// Find unsupported models
	var unsupportedModels []string
	for _, m := range allModels {
		if !supportedMap[m] {
			unsupportedModels = append(unsupportedModels, m)
		}
	}

	t.Logf("Command: bgp_config")
	t.Logf("Supported models (%d): %v", len(supportedModels), supportedModels)
	t.Logf("Unsupported models (%d): %v", len(unsupportedModels), unsupportedModels)

	// Test: Verify supported models are recognized
	for _, model := range supportedModels {
		t.Run("supported_"+model, func(t *testing.T) {
			if !IsModelSupported("bgp_config", model) {
				t.Errorf("Model %s should be supported for bgp_config", model)
			}
		})
	}

	// Test: Verify unsupported models are rejected
	for _, model := range unsupportedModels {
		t.Run("unsupported_"+model, func(t *testing.T) {
			if IsModelSupported("bgp_config", model) {
				t.Errorf("Model %s should NOT be supported for bgp_config", model)
			}
		})
	}
}
