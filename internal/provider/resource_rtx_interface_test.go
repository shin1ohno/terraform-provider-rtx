...
func TestBuildInterfaceConfigFromResourceData_Minimal(t *testing.T) {
	// Create a mock ResourceData with minimal fields
	resource := resourceRTXInterface()
	d := schema.TestResourceDataRaw(t, resource.Schema, map[string]interface{}{
		"name": "lan1",
	})
	...
	if config.Name != "lan1" {
		t.Errorf("Name = %q, want %q", config.Name, "lan1")
	}
	if config.Description != "" {
		t.Errorf("Description should be empty, got %q", config.Description)
	}
}
